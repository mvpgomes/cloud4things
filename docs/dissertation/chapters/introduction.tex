...
\begin{itemize}
  ...
  % Deployment
  \item \textbf{Deployment} of smart places usually is performed in a physical, isolated and
  vertical manner where hardware, networks, middleware and application logics are tightly coupled.
  This provisioning model presents some limitations that makes the deployment of a smart place an
  inefficient process, since that each time that a smart place is deployed its software components
  and embedded devices need to be manually installed and configured.
  % Management
